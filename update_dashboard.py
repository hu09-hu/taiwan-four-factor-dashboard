...
.factor-title{{margin:30px 0 4px;font-size:20px}}.factor-note{{margin-bottom:10px;color:var(--muted);font-size:13px}}.factor-grid{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:22px 24px;margin-bottom:28px}}.factor-panel h3{{font-size:16px;font-weight:600;margin:0 0 3px}}.factor-mini{{width:100%;height:250px}}@media(max-width:780px){{.factor-grid{{grid-template-columns:1fr;gap:14px}}.factor-mini{{height:235px}}.factor-title{{font-size:18px;margin-top:26px}}}}
...
<section class="page" data-page="model"><h1>四因子資金壓力模型</h1><div class="muted">外資現貨 × 近5日外資淨空增加 × USD/TWD × 加權指數</div><div id="model-cards" class="grid"></div>{f'<div class="warning">{warning}</div>' if warning else ''}<h2 class="factor-title">四因子分數趨勢</h2><div class="factor-note">淡線為每日分數，粗線為 5 日平均；正值代表資金順風，負值代表資金壓力。</div><div class="factor-grid"><div class="factor-panel"><h3>外資現貨</h3><div id="factor-foreign" class="factor-mini"></div></div><div class="factor-panel"><h3>淨空增加</h3><div id="factor-short" class="factor-mini"></div></div><div class="factor-panel"><h3>匯率</h3><div id="factor-fx" class="factor-mini"></div></div><div class="factor-panel"><h3>大盤指數</h3><div id="factor-market" class="factor-mini"></div></div></div><div id="score-chart" class="chart"></div></section>
...
const factorDefs=[['factor-foreign','外資現貨',0,C.blue],['factor-short','淨空增加',1,C.red],['factor-fx','匯率',2,C.yellow],['factor-market','大盤指數',3,C.violet]];
const factorDates=dates(M),factorRange=[factorDates[0],last(factorDates)];
factorDefs.forEach(([id,name,index,color])=>{{const raw=M.map(r=>r.factor_scores[index]),trend=ma(raw,5),latestTrend=last(trend);const mini=layout('分數');mini.height=250;mini.margin={{l:44,r:14,t:10,b:38}};mini.showlegend=false;mini.hovermode='x';mini.xaxis={{gridcolor:'#20314a',range:factorRange}};mini.yaxis={{title:'分數',gridcolor:'#20314a',zerolinecolor:'#64748b',range:[-105,105],fixedrange:true}};mini.shapes=[{{type:'rect',xref:'paper',x0:0,x1:1,yref:'y',y0:0,y1:100,fillcolor:'rgba(16,185,129,.035)',line:{{width:0}},layer:'below'}},{{type:'rect',xref:'paper',x0:0,x1:1,yref:'y',y0:-100,y1:0,fillcolor:'rgba(239,68,68,.035)',line:{{width:0}},layer:'below'}}];document.getElementById(id).setAttribute('aria-label',name+'因子每日分數與5日平均');Plotly.newPlot(id,[{{x:factorDates,y:raw,name:'每日分數',type:'scatter',mode:'lines',opacity:.28,line:{{color,width:1}},hovertemplate:name+'<br>%{{x}}<br>每日 %{{y:+.1f}}<extra></extra>'}},{{x:factorDates,y:trend,name:'5日平均',type:'scatter',mode:'lines',line:{{color,width:3}},hovertemplate:name+'<br>%{{x}}<br>5日平均 %{{y:+.1f}}<extra></extra>'}},{{x:[last(factorDates)],y:[latestTrend],name:'最新',type:'scatter',mode:'markers+text',marker:{{color,size:9}},text:[signed(latestTrend,1)],textposition:'middle left',hovertemplate:name+'<br>%{{x}}<br>5日平均 %{{y:+.1f}}<extra></extra>'}}],mini,{{responsive:true,displaylogo:false,modeBarButtonsToRemove:['lasso2d','select2d']}})}});
